...
class TransactionsService
{
	...
	public static function newTransaction($data) {
		global $DBH;
        if (isset($data->KidID) && isset($data->ParentID) && isset($data->ActionID) && isset($data->BikeOutID) && isset($data->BikeInID) && isset($data->Caution) && isset($data->Membership) && isset($data->Date) ) {
            try {
                $STH = $DBH->prepare("INSERT INTO " . TableService::getTable(TableEnum::TRANSACTIONS) . " (KidID, ParentID, ActionID, BikeOutID, BikeInID, Caution, Membership, Date) VALUES (:KidID, :ParentID, :ActionID, :BikeOutID, :BikeInID, :Caution, :Membership, :Date)");
