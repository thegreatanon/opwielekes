...
class MembersService
{
	...
	public static function updateParentStatus($data) {
		...
		global $DBH;
        if (isset($data->ID) && isset($data->Name) && isset($data->Surname) && isset($data->Street) && isset($data->StreetNumber) && isset($data->Postal) && isset($data->Town) && isset($data->Email) && isset($data->Phone) && isset($data->InitDate) && isset($data->Active)) {
			try {
	...
	public static function newKid($data,$parentID) {
		global $DBH;
        if (isset($parentID) && isset($data->Name) && isset($data->Surname) &&  isset($data->BirthDate) && isset($data->Caution) && isset($data->ExpiryDate) && isset($data->Active)) {
            try {
                $STH = $DBH->prepare("INSERT INTO " . TableService::getTable(TableEnum::KIDS) . " (ParentID, Name, Surname, BirthDate, Caution, ExpiryDate, Active) VALUES (:ParentID, :Name, :Surname, :BirthDate, :Caution, :ExpiryDate, :Active)");
	...
	public static function updateKidStatus($data) {
		...
		global $DBH;
        if (isset($data->ID) && isset($data->Caution) && isset($data->ExpiryDate) &&  isset($data->Active)) {
			try {
                $STH = $DBH->prepare("UPDATE " . TableService::getTable(TableEnum::KIDS) . " SET Caution = :Caution, ExpiryDate = :ExpiryDate, Active = :Active WHERE ID = :ID");
				$STH->bindParam(':ID', $data->ID);
				$STH->bindParam(':Caution', $data->Caution);
				$STH->bindParam(':ExpiryDate', $data->ExpiryDate);
				$STH->bindParam(':Active', $data->Active);
